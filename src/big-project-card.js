...
class BigProjectCard extends PolymerElement {
  static get template() {
    ...
    <paper-dialog id="dialog" modal="" entry-animation="scale-up-animation">
      ...
      <div class="buttons">
        <!-- Buttons with actions related to the current project-->
...
`;
  ...
  _projectChanged(prj, lang, repoRoot) {
    ...
    // Replace newlines with '<br>' in description only when no HTML tags are present
    const desc = (prj && lang && prj.description) ? prj.description[lang] : '';
    this.$.description.innerHTML = /<\w*>/.test(desc) ? desc : desc.replace(/\n/g, '<br>\n');
